package org.cs.module.core.dao.impl;
...
public class BaseDao<T> implements IBaseDao<T> {
	...
	@Override
	public int execute(final String sql, final String type, final Map<String, String> params) {
		return hibernateTemplate.execute(new HibernateCallback<Integer>() {
			@Override
			public Integer doInHibernate(Session session) throws HibernateException {
				if (StringUtil.isBlank(type) || type.equals("sql")) {
	...
	private static String getRowCountSql(String sql) {
		if (sql != null) {
			sql = "select count(*) from  (" + clearSql(sql) + ") aa";
		}
		return sql;
	}
	...
	@Override
	public Object insert(Object T, Class<T> clazz) throws DataAccessException {
