...
public class SessionInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request,
        ...
        if(StringUtil.isBlank(referer) && url.indexOf("login.shtml")!=-1) {            
        	...
        	return false;
        }else if(StringUtil.isBlank(referer) && url.indexOf("login_xiyuan.shtml")!=-1) {            
        	response.sendRedirect(contextPath + "/login_xiyuan.jsp");  
//        	response.sendRedirect(contextPath+"/index.jsp");
        	return false;
        }
        ...
        if(objUser == null){//未登录，或者登录超时
            ...
            if(url.indexOf("getVotePage.shtml")!=-1){
            	response.sendRedirect(contextPath+"/login_xiyuan.jsp");
//            	response.sendRedirect(contextPath+"/index.jsp");
            }else{
            	response.sendRedirect(contextPath+"/index.jsp");
